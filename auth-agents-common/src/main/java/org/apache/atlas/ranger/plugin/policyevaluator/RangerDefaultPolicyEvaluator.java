...
package org.apache.atlas.ranger.plugin.policyevaluator;
...
import org.apache.atlas.ranger.plugin.conditionevaluator.RangerAbstractConditionEvaluator;
import org.apache.atlas.ranger.plugin.conditionevaluator.RangerConditionEvaluator;
...
import org.apache.atlas.ranger.plugin.model.RangerPolicy.RangerRowFilterPolicyItem;
import org.apache.atlas.ranger.plugin.model.RangerServiceDef;
import org.apache.atlas.ranger.plugin.model.RangerServiceDef.RangerAccessTypeDef;
...
import org.apache.atlas.ranger.plugin.policyengine.RangerAccessResult;
import org.apache.atlas.ranger.plugin.policyengine.RangerPolicyEngine;
import org.apache.atlas.ranger.plugin.policyengine.RangerPolicyEngineOptions;
import org.apache.atlas.ranger.plugin.policyengine.RangerResourceAccessInfo;
import org.apache.atlas.ranger.plugin.policyengine.RangerTagAccessRequest;
import org.apache.atlas.ranger.plugin.policyresourcematcher.RangerDefaultPolicyResourceMatcher;
import org.apache.atlas.ranger.plugin.policyresourcematcher.RangerPolicyResourceMatcher;
import org.apache.atlas.ranger.plugin.resourcematcher.RangerResourceMatcher;
	...
	*/
	...
	protected void evaluatePolicyItems(RangerAccessRequest request, RangerPolicyResourceMatcher.MatchType matchType, RangerAccessResult result) {
		...
		if (useAclSummaryForEvaluation && (getPolicy().getPolicyType() == null || getPolicy().getPolicyType() == RangerPolicy.POLICY_TYPE_ACCESS)) {
			...
			if (accessResult != null) {
				updateAccessResult(result, matchType, accessResult.equals(ACCESS_ALLOWED), null);
	...
	private Integer lookupAccess(String userOrGroup, String accessType, Map<String, PolicyACLSummary.AccessResult> accesses) {
		...
		if (accesses != null) {
			if (accessType.equals(RangerPolicyEngine.ANY_ACCESS)) {
				...
				if (accessResult != null) {
					if (accessResult.getResult() == ACCESS_CONDITIONAL) {
	...
	private Integer getAccessResultForAnyAccess(Map<String, PolicyACLSummary.AccessResult> accesses) {
		...
		for (Map.Entry<String, PolicyACLSummary.AccessResult> entry : accesses.entrySet()) {
			...
			if (accessResult.getResult() == ACCESS_ALLOWED) {
				allowedAccessCount++;
				break;
			} else if (accessResult.getResult() == ACCESS_DENIED) {
				deniedAccessCount++;
			}
		}
	...
	protected boolean isAccessAllowed(String user, Set<String> userGroups, Set<String> roles, String owner, String accessType) {
		...
		if (useAclSummaryForEvaluation && (getPolicy().getPolicyType() == null || getPolicy().getPolicyType() == RangerPolicy.POLICY_TYPE_ACCESS)) {
			...
			if (accessResult != null && accessResult.equals(ACCESS_ALLOWED)) {
				ret = true;
			}
		} else {
			if (LOG.isDebugEnabled()) {
