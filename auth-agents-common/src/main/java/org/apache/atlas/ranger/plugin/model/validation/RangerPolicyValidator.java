...
import org.apache.atlas.ranger.plugin.model.RangerPolicyResourceSignature;
...
public class RangerPolicyValidator extends RangerValidator {
	...
	boolean isValidAccessTypeDef(RangerPolicy policy, final List<ValidationFailureDetails> failures, Action action,boolean isAdmin, final RangerServiceDef serviceDef) {
		...
		String policyType = StringUtils.isEmpty(policy.getPolicyType()) ? RangerPolicy.POLICY_TYPE_ACCESS : policy.getPolicyType();
		//row filter policy
		if (RangerPolicy.POLICY_TYPE_ROWFILTER.equals(policyType)) {
		...
		//data mask policy
		if (RangerPolicy.POLICY_TYPE_DATAMASK.equals(policyType)){
