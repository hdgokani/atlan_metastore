    ...
    */
    ...
    private void traverseEdgesUsingBFS(String baseGuid, AtlasLineageListContext lineageListContext, AtlasLineageListInfo ret, String lineageType) throws AtlasBaseException {
        ...
        boolean isNotConnecterVertex =  lineageType.equals(PRODUCT_ASSET_LINEAGE)
                ? dataTypeMap.get(IS_DATA_PRODUCT)
                : dataTypeMap.get(IS_DATASET);
        enqueueNeighbours(baseVertex, dataTypeMap, lineageListContext, traversalQueue, visitedVertices, skippedVertices, lineageType, lineageParentsForEntityMap, lineageChildrenForEntityMap);
        int currentDepth = 0;
        ...
        while (!traversalQueue.isEmpty() && !lineageListContext.isEntityLimitReached() && currentDepth < lineageListContext.getDepth()) {
            ...
            for (int i = 0; i < entitiesInCurrentDepth; i++) {
                ...
                if (!lineageListContext.evaluateVertexFilter(currentVertex)) {
                    enqueueNeighbours(currentVertex, currentEntityDataTypeMap, lineageListContext, traversalQueue, visitedVertices, skippedVertices, lineageType, lineageParentsForEntityMap, lineageChildrenForEntityMap);
                    continue;
                }
    ...
    private void enqueueNeighbours(AtlasVertex currentVertex, HashMap<String, Boolean> dataTypeMap, AtlasLineageListContext lineageListContext,
                                   Queue<String> traversalQueue, Set<String> visitedVertices, Set<String> skippedVertices,String lineageType, Map<String, List<String>> lineageParentsForEntityMap, Map<String, List<String>> lineageChildrenForEntityMap) {
